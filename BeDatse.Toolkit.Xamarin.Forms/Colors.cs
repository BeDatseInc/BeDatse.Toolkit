...
namespace BeDatse.Toolkit.Xamarin.Forms
{
    class Colors
    {
        private Colors()
        ...
        static Colors()
        {
            ...
            foreach (var fieldInfo in typeof(Colors).GetRuntimeFields())
            {
                if (fieldInfo.IsPublic && fieldInfo.IsStatic && fieldInfo.FieldType == typeof(Color))
                {
                    ...
                    var createColor = new Colors
                    {
                        Name = name,
                        FriendlyName = stringBuilder.ToString(),
                        Color = color,
                        RgbDisplay =
                            $"{(int) (255*color.R):X2}-{(int) (255*color.G):X2}-{(int) (255*color.B):X2}"
                    };
        ...
        public static IList<Colors> All { get; private set; }
